...
/**
 * @deprecated This function is not currently used by any route handler.
 * Consider removing it or integrating it into the AI routes.
 */
export function selectModel(userMessage: string, requestedModel?: string): string {
  if (requestedModel && (ALLOWED_MODELS as readonly string[]).includes(requestedModel)) return requestedModel;
