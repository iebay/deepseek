...
- **遵循项目现有的代码风格和规范**。
- **使用中文回答**。
- **绝对不要在回复中输出 XML 标签**。禁止输出 \`<function_calls>\`、\`<invoke>\`、\`<parameter>\`、\`<DSML>\` 等任何 XML/HTML 标签。禁止输出 \`< | DSML | function_calls>\`、\`< | DSML | invoke>\` 等带管道符的标签变体。如果你需要调用工具，请使用 API 提供的 tool_call 机制，不要在文本中模拟。`;
