...
function EventCard({ entry, onToggle }: { entry: EventEntry; onToggle: () => void }) {
  const bgMap: Record<string, string> = {
    ...
    tool_result: 'bg-[#161b22] border-[#30363d]',
    content: 'bg-[#0d1117] border-[#30363d]',
    done: 'bg-[#0d2616] border-[#1a5c2e]',
    error: 'bg-[#2d0d0d] border-[#6e1212]',
  };
  const iconMap: Record<string, JSX.Element> = {
    thinking: <Brain size={13} className="text-[#58a6ff] shrink-0" />,
    tool_call: <Wrench size={13} className="text-[#bc8cff] shrink-0" />,
    tool_result: <FileText size={13} className="text-[#8b949e] shrink-0" />,
    content: <Bot size={13} className="text-[#388bfd] shrink-0" />,
    done: <CheckCircle2 size={13} className="text-[#3fb950] shrink-0" />,
    error: <XCircle size={13} className="text-[#f85149] shrink-0" />,
  };

  const bg = bgMap[entry.type] || 'bg-[#161b22] border-[#30363d]';
  ...
  function renderContent() {
    switch (entry.type) {
      case 'thinking':
        return <span className="text-[#8b949e]">{entry.message}</span>;
      case 'tool_call':
        return (
          <div>
            <span className="text-[#bc8cff] font-mono text-xs">{entry.tool}</span>
            {entry.args && Object.keys(entry.args).length > 0 && (
              <pre className="mt-1 text-[10px] text-[#8b949e] overflow-auto max-h-20 whitespace-pre-wrap break-all">
      ...
      case 'tool_result':
        if (entry.collapsed) {
          return <span className="text-[#8b949e] text-xs italic">（点击展开结果）</span>;
        }
        return (
      ...
      case 'content':
        return <p className="text-[#e6edf3] whitespace-pre-wrap break-words text-xs">{entry.content}</p>;
      case 'done':
        return <span className="text-[#3fb950]">任务完成（共 {entry.iterations} 步）</span>;
      case 'error':
        return <span className="text-[#f85149]">{entry.message}</span>;
      default:
        return null;
    }
  }
...
export default function AgentPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0d1117] text-[#e6edf3]">
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[#30363d] shrink-0">
        <Bot size={15} className="text-[#388bfd]" />
        <span className="text-sm font-semibold text-[#e6edf3]">Agent 模式</span>
        <span className="ml-auto text-[10px] text-[#8b949e] bg-[#21262d] px-1.5 py-0.5 rounded-full">
          {selectedModel}
        </span>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-2 space-y-2 min-h-0">
        {events.length === 0 && !isRunning && (
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 text-[#8b949e]">
            <Bot size={32} className="text-[#388bfd]/40" />
            <div>
              <p className="text-sm font-medium text-[#e6edf3]/70">AI 代码代理</p>
        ...
        {isRunning && (
          <div className="flex items-center gap-2 text-xs text-[#8b949e] px-2">
      ...
      <div className="border-t border-[#30363d] p-2 shrink-0">
        <div className="flex gap-2 items-end">
          <textarea
            ...
            rows={2}
            className="flex-1 bg-[#161b22] border border-[#30363d] rounded-lg px-3 py-2 text-sm text-[#e6edf3] placeholder-[#8b949e] focus:outline-none focus:border-[#388bfd] resize-none disabled:opacity-50 transition-colors"
          />
          {isRunning ? (
            <button
              onClick={handleStop}
              className="p-2 rounded-lg bg-[#f85149]/10 hover:bg-[#f85149]/20 text-[#f85149] transition-colors shrink-0"
              title="停止"
            >
              <Square size={15} />
            </button>
            ...
            <button
              ...
              className="p-2 rounded-lg bg-[#388bfd]/10 hover:bg-[#388bfd]/20 text-[#388bfd] disabled:opacity-50 transition-colors shrink-0"
              title="发送 (Enter)"
            >
              <Send size={15} />
            </button>
        ...
        <p className="text-[10px] text-[#484f58] mt-1 px-1">Enter 发送 · Shift+Enter 换行</p>
      </div>
    </div>
  );
}
