      ...
      <div className="px-3 py-2 flex items-center gap-2">
        ...
        <div className="relative flex-1 min-w-0" ref={dropdownRef}>
          <button
            ...
            className="flex items-center gap-1.5 w-full min-w-0 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg px-2 py-1 text-xs hover:border-[#388bfd] transition-colors"
          >
            ...
            <div className="absolute top-full left-0 mt-1 w-full bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-lg shadow-xl z-50 max-h-48 overflow-y-auto">
              {branches.local.map((b) => (
                <button
                  ...
                  className={`flex items-center gap-2 w-full px-2.5 py-1.5 text-xs hover:bg-[var(--bg-hover)] transition-colors ${b.isCurrent ? 'cursor-default' : 'cursor-pointer'}`}
                >
                  {b.isCurrent
        ...
        <button
          onClick={handlePull}
          disabled={isPulling}
          className="p-1.5 rounded-lg text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] transition-colors disabled:opacity-50"
          title="拉取 (Pull)"
        >
            ...
            <ul className="pb-1">
              {branches.remote.map((r) => (
                <li key={r.name} className="flex items-center gap-2 px-3 py-1 text-xs text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)] transition-colors">
                  <span className="w-1.5 h-1.5 rounded-full bg-[var(--text-secondary)] shrink-0" />
          ...
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-xl p-4 w-80 shadow-xl">
            ...
            <div className="flex gap-2 justify-end">
              <button
                ...
                className="text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)] px-3 py-1.5 rounded-lg hover:bg-[var(--bg-hover)] transition-colors"
              >
                取消
              </button>
              <button
...
function BranchRow({ branch, currentBranch, onCheckout, onDelete, onMerge }: BranchRowProps) {
  ...
  return (
    ...
    >
      {isCurrent
        ? <span className="w-1.5 h-1.5 rounded-full bg-[var(--success)] shrink-0" />
        : <span className="w-1.5 h-1.5 rounded-full border border-[var(--text-secondary)] shrink-0" />
      }
      <button
        onClick={!isCurrent ? onCheckout : undefined}
        className={`truncate text-xs flex-1 text-left ${isCurrent ? 'text-[var(--success)] cursor-default' : 'text-[var(--text-primary)] hover:text-[#58a6ff] cursor-pointer'}`}
      ...
      {hovered && !isCurrent && (
        <div className="flex items-center gap-0.5 shrink-0">
          <button
            onClick={onMerge}
            className="p-1 rounded text-[var(--text-secondary)] hover:text-[#bc8cff] hover:bg-[var(--bg-hover)] transition-colors"
          ...
          <button
            onClick={onDelete}
            className="p-1 rounded text-[var(--text-secondary)] hover:text-[var(--error)] hover:bg-[var(--bg-hover)] transition-colors"
            title="删除分支"
          >
            <Trash2 size={11} />
          </button>
