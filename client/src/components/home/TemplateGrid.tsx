    ...
    <div className="grid grid-cols-2 gap-3">
      {templates.map((tpl) => (
        <button
          ...
          className="group relative text-left p-4 rounded-xl border border-[var(--border-primary)] hover:border-[var(--accent-border)] bg-[var(--bg-secondary)] hover:bg-[var(--bg-tertiary)] transition-all duration-200 overflow-hidden"
        >
          {/* Gradient background */}
          ...
          <div className="relative">
            ...
            <div className="flex flex-wrap gap-1">
              {tpl.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="text-[10px] px-1.5 py-0.5 bg-[var(--accent-bg)] text-[var(--accent-hover)] rounded-md border border-[var(--accent-border)]"
                >
                  {tag}
                </span>
              ))}
