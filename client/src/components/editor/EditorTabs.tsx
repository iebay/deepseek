  ...
  return (
    <>
      <div className="h-9 bg-[var(--bg-primary)] border-b border-[var(--border-primary)] flex items-center overflow-x-auto shrink-0 scrollbar-thin">
        {openTabs.map((tab, index) => (
          <div
            ...
            className={`flex items-center gap-1.5 px-3 h-full border-r border-[var(--border-primary)] cursor-pointer text-xs shrink-0 group transition-colors select-none ${
              tab.path === activeTabPath
                ? 'bg-[var(--editor-tab-active-bg)] text-[var(--text-primary)] border-t-2 border-t-[var(--accent-primary)]'
          ...
          >
            ...
            <button
              className="opacity-0 group-hover:opacity-100 hover:text-[var(--error)] hover:bg-[var(--error-bg)] rounded p-0.5 transition-all -mr-1"
      ...
      {contextMenu && (
        ...
        >
          <button
            className="flex items-center gap-2 w-full px-3 py-2 text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-colors text-left text-xs"
